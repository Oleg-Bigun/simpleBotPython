...
class Booking(webdriver.Chrome):
    def __init__(self, driver_path=r"/Users/hhhhh/ChromeDriver", teardown=False):
        self.driver_path = driver_path
        self.teardown = teardown
        os.environ["PATH"] += self.driver_path
        ch_options = webdriver.ChromeOptions()
        ch_options.add_experimental_option("detach", True)
        super(Booking, self).__init__(options=ch_options)
    ...
    def change_currency(self, currency=None):
        ...
        selected_currency_element = \
            self.find_element(By.CSS_SELECTOR, f'a[data-modal-header-async-url-param*="selected_currency={currency}"]'
                              )
